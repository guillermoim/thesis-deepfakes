...
from .transforms_bank import remove_landmark, prepare_bit_masks
...
class ImagesDataset(Dataset):
    def __init__(self, video_dirs, name, target, max_images_per_video, max_videos, transform, window_size, normalization):
        self.name = name
    ...
    def _get_item(self, index):
        ...
        target = self.target
        image = np.asarray(Image.open(img['img_path']), dtype=np.uint8).copy()

        landmarks = None
        with torch.no_grad():
            _, _, lands = self.mtcnn.detect(image, landmarks=True)
            landmarks = lands[0] if lands is not None else None
...
def read_train_test_val_dataset(dataset_dir, name, target, splits_path, **dataset_kwargs):
...
def read_dataset(data_dir, normalization, transform, max_videos, window_size, max_images_per_video, splits_path='../dataset/splits/'):
